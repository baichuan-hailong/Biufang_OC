...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        //------footer位置改变（biufang-myself）------
        CGFloat arrowX = 32 - ZY_ARROW_SIDE - 5;
        CGFloat arrowY = self.imageHeight / 2 - ZY_ARROW_SIDE / 2;
        CGFloat arrowW = ZY_ARROW_SIDE;
        CGFloat arrowH = ZY_ARROW_SIDE;
        self.footer.arrowView.frame = CGRectMake(arrowX, arrowY, arrowW, arrowH);
        
        CGFloat labelX = 32 + 2;
        CGFloat labelY = (self.imageHeight - 150) / 2;
        CGFloat labelW = ZY_ARROW_SIDE;
        CGFloat labelH = 150;
        self.footer.label.frame = CGRectMake(labelX, labelY, labelW, labelH);
        //------------------------------------------
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_autoScroll || _shouldLoop) {
        self.pageControl.currentPage = (NSInteger)((scrollView.contentOffset.x/self.flowLayout.itemSize.width)) % self.itemCount;
    } else {
        self.pageControl.currentPage = (NSInteger)((scrollView.contentOffset.x + SCREEN_WIDTH - self.flowLayout.itemSize.width)/self.flowLayout.itemSize.width);
    }
    ...
    CGFloat footerDisplayOffset = (scrollView.contentOffset.x - (self.flowLayout.itemSize.width * (self.itemCount - 1))) + SCREEN_WIDTH - _flowLayout.itemSize.width;
...
- (UIPageControl *)pageControl
{
    if (!_pageControl) {
        ...
        _pageControl.userInteractionEnabled = NO;
        _pageControl.autoresizingMask = UIViewAutoresizingNone;
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithHex:RED_COLOR];
        _pageControl.pageIndicatorTintColor = [UIColor colorWithHex:@"bdbdbd"];
    }
    return _pageControl;
}
